...
public class CompraDAO {
	...
	public static void create(Compra c) {
		...
		try {
			...
			if (c.getVl_desconto() == 0)
				stmt.setNull(2, Types.DOUBLE);
			else
				stmt.setDouble(2, c.getVl_desconto());
			if (c.getVl_imposto() == 0)
				stmt.setNull(3, Types.DOUBLE);
			else
				stmt.setDouble(3, c.getVl_imposto());
			stmt.setDate(4, c.getDt_compra());
			if (c.getVl_comissao() == 0)
				stmt.setNull(5, Types.DOUBLE);
			else
				stmt.setDouble(5, c.getVl_comissao());
