	...
	public static void create (Encomenda e) throws SQLException {
		...
		try {
			stm = con.prepareStatement("INSERT INTO encomenda "
					+ "(id_encomenda, dt_pedido, status_encomenda, dt_prevista,cpf_cliente) "
					+ "VALUES(?,?,?,?,?)");
...
//			con.close();
	...
	public static void remove(int id_encomenda) throws SQLException {
		...
		PreparedStatement stm = null;
		try {
			stm = con.prepareStatement("DELETE FROM encomenda "
					+ "WHERE id_encomenda = ?");
...
//			con.close();
	...
	public static void update (Encomenda e) throws SQLException {
		...
		PreparedStatement stm = null;
		try {
			stm = con.prepareStatement("UPDATE encomenda "
					+ "SET status_encomenda = ?, dt_prevista = ? "
					+ "WHERE id_encomenda = ?");
